...
simulated function RefreshScoreboard()
{
    if(IsJuggernaut()){
        AOCBaseHUD(AOCPlayerController(Controller).myHUD).HUD.Scoreboard.SetBottomLeftText("You are the Juggernaut!<br/>");
    }else{
         AOCBaseHUD(AOCPlayerController(Controller).myHUD).HUD.Scoreboard.SetBottomLeftText("<br/>");
    }
}
...
simulated function SetGiantScale(float NewScale)
{
    ...
    {
        SetTimer(2.0f, false, 'RefreshScoreboard');
        RefreshScoreboard();
    }
    ...
    //JuggernautPlayerController(Controller).ClientDisplayConsoleMessage("Old height: "@GetCollisionHeight());
    ...
    //JuggernautPlayerController(Controller).ClientDisplayConsoleMessage("New height: "@GetCollisionHeight());
...
singular event BaseChange()
{
    ...
    if (JuggernautPawn(Base) != None && JuggernautPawn(Base).GiantScale <= GiantScale / CrushDifferenceThreshold)
    {
...
function bool IsJuggernaut(){
    return GiantScale == 2;
}

function CrushedBy(Pawn OtherPawn)
{
    if(JuggernautPawn(OtherPawn).GiantScale >= CrushDifferenceThreshold * GiantScale)
    {
        PlaySound(SoundCue'A_Impacts_Melee.Giant_stomped', FALSE, FALSE, TRUE);
...
event TakeDamage(int Damage, Controller InstigatedBy, vector HitLocation, vector Momentum, class<DamageType> DamageType, optional TraceHitInfo myHitInfo, optional Actor DamageCauser)
{
    ...
    local JuggernautPawn EnemyPawn;
    
    EnemyPawn = JuggernautPawn(DamageCauser);
    if(EnemyPawn == none)
    {
        EnemyPawn = JuggernautPawn(InstigatedBy.Pawn);
    }
...
reliable server function AttackOtherPawn(HitInfo Info, string DamageString, optional bool bCheckParryOnly = false, optional bool bBoxParrySuccess, optional bool bHitShield = false, optional SwingTypeImpactSound LastHit = ESWINGSOUND_Slash, optional bool bQuickKick = false)
{
    ...
    bCannotFlinchDueToScale = JuggernautPawn(Info.HitActor).GiantScale/GiantScale >= FlinchImpossibilityThreshold;
    ...
    //{
        ...
        if (JuggernautPawn(Info.HitActor).GiantScale/GiantScale >= ParryImpossibilityThreshold || GiantScale / JuggernautPawn(Info.HitActor).GiantScale >= ParryImpossibilityThreshold)
        {
            bParry = false;
        }
    ...
    if (PawnFamily.FamilyFaction == Info.HitActor.PawnFamily.default.FamilyFaction)
